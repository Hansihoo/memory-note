...
- Q1 policy revision done: removed the mandatory new-word quota from daily quest policy, kept the existing queue/recommendation algorithm as the source of what to check next, changed quest completion to 25 card review attempts, and kept distinct `todayStudiedCount` separate from quest `completedCount`. Files changed: `.agent/DECISIONS.md`, `.agent/MEMORY_NOTE_TASKS.md`, `apps/api/app/main.py`, `apps/api/tests/test_api.py`, `apps/extension/src/popup.ts`, `apps/extension/src/popup.test.ts`. Validation: `pnpm run ci` passed.
- Q1 profile/extension stats done: exposed `dailyQuestCompletedCount`, `dailyQuestCompleted`, `memorizedWordCount`, and a read-only memorized word list through additive profile/daily quest fields. Profile and Chrome popup now show daily quest completion count and memorized word count, and clicking the memorized word stat reveals the memorized word list. Completion count is computed from `review_logs` days with at least 25 review attempts and does not mutate learning state. Validation: `pnpm run ci` passed.

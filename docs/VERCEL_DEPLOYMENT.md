...
The FastAPI app is exposed from both `main.py` and `api/index.py` so Vercel can use its zero-config Python/FastAPI detection regardless of which entrypoint it resolves first. `vercel.json` rewrites `/health` to a small standalone health function and all other requests to `api/index.py`.

`fastapi` and `pydantic` are pinned to Python-runtime-compatible versions in `requirements.txt`. If Vercel logs show `ForwardRef._evaluate()` during import, redeploy after confirming those pins are installed by the latest build.

...
- 새 계정에는 기본 샘플로 `암기 노트 기본 단어` 100개와 `암기 노트 기본 문장` 100개가 자동 생성된다.
...
- 노트 우측 상단에는 스피커 아이콘이 있으며, 켜면 현재 문제와 공개된 답을 읽어준다.
- 읽어주기 기능은 교체 가능한 speech driver 구조를 사용하며, 현재 웹 구현은 브라우저 내장 TTS를 사용한다.
...
- `프로필` 화면에서 누적 학습일, 오늘 학습한 단어 수, 외운 단어 수와 외운 단어 목록을 확인한다.
